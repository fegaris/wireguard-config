...
function installQuestions() {
	...
	echo ""
	echo "Necesito que respondas algunas preguntas sobre la configuración"
	echo "Puedes dejar las opciones por defecto si pulsas enter"
	echo ""
	...
	read -rp "Introduce la dirección IPv4 o IPv6 PUBLICA del servidor: " -e -i "${SERVER_PUB_IP}" SERVER_PUB_IP
	...
	until [[ ${SERVER_WG_IPV4} =~ ^([0-9]{1,3}\.){3} ]]; do
		read -rp "Direccion IPv4 del servidor: " -e -i 10.66.66.1 SERVER_WG_IPV4
	done

	until [[ ${SERVER_WG_IPV6} =~ ^([a-f0-9]{1,4}:){3,4}: ]]; do
		read -rp "Direccion IPv6 del servidor: " -e -i fd42:42:42::1 SERVER_WG_IPV6
	done

	# Generate random number within private ports range
	RANDOM_PORT=$(shuf -i49152-65535 -n1)
	until [[ ${SERVER_PORT} =~ ^[0-9]+$ ]] && [ "${SERVER_PORT}" -ge 1 ] && [ "${SERVER_PORT}" -le 65535 ]; do
		read -rp "Puerto del servidor WireGuard [1-65535]: " -e -i "51820" SERVER_PORT
	done
	...
	echo "Ya puedes crear clientes para el servidor"
...
function installSmtpQuestions() {
	...
	SMTP_PASSWORD=""
	echo ""
	echo "Vamos a configurar el envío de notificaciones por email"
	echo ""

	read -rp "Introduce la cuenta: " -e -i "${SMTP_ACCOUNT}" SMTP_ACCOUNT
	read -rp "Introduce el host: " -e -i "${SMTP_HOST}" SMTP_HOST
	read -rp "Introduce el puerto: " -e -i "${SMTP_PORT}" SMTP_PORT
	read -rp "Introduce la cuenta de correo desde la que quieras enviar los emails: " -e -i "${SMTP_EMAIL}" SMTP_EMAIL
	read -rp "Introduce la contraseña de la cuenta de correo: " -e -i "${SMTP_PASSWORD}" SMTP_PASSWORD
...
function installSmtp() {
	...
	# Configuración para usar el SMTP de Gmail
	touch "/root/.msmtprc"
	...
	echo "logfile ~/.msmtp.log" >> "/root/.msmtprc"
	#Account gmail
	echo "account $SMTP_ACCOUNT" >> "/root/.msmtprc"
	#Host smtp.gmail.com
	echo "host $SMTP_HOST" >> "/root/.msmtprc"
	#Port 587
	echo "port $SMTP_PORT" >> "/root/.msmtprc"
	#from and user email 
	echo "from $SMTP_EMAIL" >> "/root/.msmtprc"
	echo "user $SMTP_EMAIL" >> "/root/.msmtprc"
	echo "password $SMTP_PASSWORD" >> "/root/.msmtprc"
	echo "account default : $SMTP_ACCOUNT" >> "/root/.msmtprc"
...
function newClient() {
	...
	echo ""
	echo "Introduce un nombre para el cliente: "
	...
	#Los clientes empiezan con la IP .10 porque me gusta más
	for DOT_IP in {10..254}; do
...
AllowedIPs = ${CLIENT_WG_IPV4}/32,${CLIENT_WG_IPV6}/128" >>"/etc/wireguard/${SERVER_WG_NIC}.conf"
	...
	echo -e "\n¿Cómo quieres compartir el archivo de configuración?"
	echo "1 - Mostrar código QR"
	echo "2 - Enviar archivo por email"
	read OPTION
	...
	if [[ ! -e /root/.msmtprc ]]; then
		#source /root/.msmtprc
		installSmtpQuestions
	fi

	#Pedimos el correo y enviamos el email
	read -rp "Escribe el correo del destinatario: " SMTP_CLIENT_EMAIL
	mail -s "Archivo de configuración WireGuard" $SMTP_CLIENT_EMAIL < "${HOME_DIR}/${SERVER_WG_NIC}-client-${CLIENT_NAME}.conf"
	;;
	esac
